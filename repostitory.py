...
class Repostitory:
    ...
    def process_message_entities(self, message: Message) -> Dict[str, List[int]]:
        ...
        hash_result = self._get_message_entity_hashes(message)
    ...
    def _get_message_entity_hashes(self, message: Message) -> GetMessageEntityHashesResult:
        picture_key = None
        url_keys = list()
        ...
        if len(url_entities) > 0:
            for url_entity in url_entities:
                url_keys.append(str(hashlib.sha256(
                    bytes(message.text[url_entity.offset: url_entity.offset + url_entity.length], 'utf-8')).hexdigest()))
